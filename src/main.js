...
function renderStack() {
  ...
  for (let i = start; i < end; i++) {
    ...
    // EXPLANATORY TOOLTIP: the badge is shown when this session contains
    // at least one tool_result with `is_error: true` from Claude's
    // tool-use protocol. That is NOT the same as a runtime crash of
    // Memex itself — it just means a Bash / Edit / Read / etc. call
    // inside the session returned an error response (file not found,
    // command exit non-zero, type mismatch, etc.). The label "errors"
    // alone was being misread as "Memex itself errored"; the title
    // attribute makes the actual meaning discoverable on hover.
